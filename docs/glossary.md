...
**Approval request drawer** — The framework-canonical shape written to `notifications.pending.approvals` when TAG routes an output as `approval_required`. Carries `kind: "approval_request"`, `run_id`, `waitpoint_signal`, `decisions`, `payload_preview`, resolved channel binding, and `idempotency_key`. The outbound dispatcher picks it up; channel adapters render decisions as inline buttons. (Pre-#56 the drawer landed at `notifications.pending.waitpoints.<run_id>`; that form still drains during a rolling upgrade — the dispatcher selects by `wing+hall` only, not by room name.) See `architecture.md §13` and `packages/runtime/src/engine/apply.ts`.
...
**Workspace manifest** — Tenant registry file that is the source of truth for a client's Nexaas deployment. Contains capability bindings, channel bindings, installed agents, behavioral contract overrides, custom domain configuration, and model policy. Canonical schema: `packages/runtime/src/schemas/workspace-manifest.ts` (Zod). Lives in the consuming business's workspace directory (`/opt/nexmatic/workspaces/<id>.workspace.json` for Nexmatic deployments), not in the Nexaas framework repo — the framework is tenant-agnostic. Validated at worker startup via `loadWorkspaceManifest`; missing or malformed manifests are non-fatal (fail-open with warnings).
