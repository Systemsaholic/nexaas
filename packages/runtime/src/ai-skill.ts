...
// The server clock is UTC, which is routinely a day ahead of the workspace's
// local evening. AI-skills have no shell to run `date`, so we compute the
// authoritative local "now" in Node and hand it to the model. Timezone comes
// from the canonical resolution (workspace config → NEXAAS_TIMEZONE → UTC) —
// #277 shipped this hardcoded to one client's timezone; #260 generalized it.
const CRON_DOW = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];

export function buildRuntimeClockHeader(timeZone: string, now: Date = new Date()): string {
  ...
  return (
    ...
    `Current date & time in ${timeZone} (the workspace's local timezone): ${localDate} (${weekday}) ${parts.hour}:${parts.minute}.\n` +
    `Today's local date is ${localDate}; the local weekday is ${weekday} (cron day-of-week ${dow}, where Sun=0..Sat=6).\n` +
...
export async function runAiSkill(
  ...
  // the workspace-local date/time so every skill has one trustworthy
  // "today" without needing a shell. Purely additive; prepended to the prompt.
  systemPrompt = buildRuntimeClockHeader(await workspaceTimezone(workspace)) + systemPrompt;
